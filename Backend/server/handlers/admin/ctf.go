...
func AddChallenge(c echo.Context) error {
	...
	// check if the points are negative
	pointsi, err := strconv.Atoi(points)
	if err != nil {
		return c.JSON(500, map[string]string{"error": "Failed to parse points."})
	}
	if pointsi < 0 {
		return c.JSON(400, map[string]string{"error:": "Lab points cannot be negative."})
	}
